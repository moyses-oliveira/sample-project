...
class User extends AbstractController {
    ...
    private function form($data = [])
    {
        ...
        return $this->render(compact('data', 'action', 'profiles', 'profilesEnabled', 'groups', 'groupsEnabled', 'contactMode', 'collection', 'saved'));
    }

    public function save(?string $pk = null)
    {
        $entity = new \Data\Entity\Acl\User();
        if (!!$pk && !$entity->load($pk))
            return $this->json403();

        $response = (new \Data\Service\Acl\User())->save($entity, $_POST);
